...
import { ALL_FIELDS, FIELD_SUBJECTS } from '../data/universities';
...
function UniversityModal({ university: u, onClose }: { university: University; onClose: () => void }) {
  ...
  // Only show fields available in this university
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          <h2 style={{ color: '#ffffff', fontWeight: 900, fontSize: 22, margin: '0 0 10px', letterSpacing: '-0.4px', lineHeight: 1.2, paddingRight: 40 }}>
            {u.name}
          </h2>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' as const }}>
            <div style={{ padding: '7px 16px', borderRadius: 50, fontWeight: 800, fontSize: 12, background: 'rgba(187,247,208,0.15)', border: '1.5px solid rgba(187,247,208,0.30)', color: '#bbf7d0' }}>
              🎓 Min CGPA {u.requiredCGPA.toFixed(1)}
            </div>
            <div style={{ padding: '7px 16px', borderRadius: 50, fontWeight: 800, fontSize: 12, background: 'rgba(191,219,254,0.15)', border: '1.5px solid rgba(191,219,254,0.30)', color: '#bfdbfe' }}>
              📝 Min IELTS {u.requiredIELTS.toFixed(1)}
            </div>
            <div style={{ padding: '7px 16px', borderRadius: 50, fontWeight: 800, fontSize: 12, background: 'rgba(253,230,138,0.15)', border: '1.5px solid rgba(253,230,138,0.25)', color: '#fde68a' }}>
        ...
        <div style={{ padding: '28px 36px 32px' }}>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 11, color: N.dim, margin: '2px 0 0', fontWeight: 500 }}>
                  Choose one or more fields available at this university
                </p>
              </div>
              {selectedFields.length > 0 && (
                <span style={{ marginLeft: 'auto', fontSize: 11, background: N.blueBright, color: '#fff', borderRadius: 50, padding: '2px 10px', fontWeight: 800 }}>
                ...
                return (
                  <motion.div
                    ...
                    style={{
                      ...
                      background: active
                        ? 'linear-gradient(135deg, rgba(30,77,140,0.07), rgba(37,99,176,0.04))'
                        : '#fafbff',
                      transition: 'all 0.2s',
                    }}
                  >
                    ...
                    <div style={{
                      ...
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      background: active ? `linear-gradient(135deg, ${N.navyMid}, ${N.blueBright})` : '#ffffff',
                      border: active ? 'none' : '2px solid rgba(30,77,140,0.25)',
                      boxShadow: active ? '0 2px 8px rgba(37,99,176,0.25)' : 'none',
                      transition: 'all 0.2s',
                    }}>
                    ...
                    <span style={{ fontSize: 14, fontWeight: active ? 700 : 600, color: active ? N.navy : 'rgba(10,31,68,0.70)', flex: 1 }}>
                      {field}
                    </span>
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 11, color: N.dim, margin: '2px 0 0', fontWeight: 500 }}>
                      Subjects are organized by their fields
                    </p>
                  </div>
                  {selectedSubjects.length > 0 && (
                    <span style={{ marginLeft: 'auto', fontSize: 11, background: '#166534', color: '#fff', borderRadius: 50, padding: '2px 10px', fontWeight: 800 }}>
                  ...
                  {selectedFields.map(field => (
                    <div key={field} style={{ borderRadius: 16, border: '1px solid rgba(30,77,140,0.10)', overflow: 'hidden' }}>
                      ...
                      <div style={{
                        ...
                        <span style={{ fontSize: 12, fontWeight: 800, color: N.navyMid }}>
                          {field}
                        </span>
                      </div>

                      {/* Subjects as list */}
                      <div style={{ padding: '10px 8px', display: 'flex', flexDirection: 'column' as const, gap: 4 }}>
                          ...
                          return (
                            ...
                            >
                              <div style={{
                                ...
                                border: active ? 'none' : '1.5px solid rgba(30,77,140,0.20)',
                                transition: 'all 0.15s',
                              }}>
                                {active && <span style={{ color: '#fff', fontSize: 10, fontWeight: 900 }}>✓</span>}
                              </div>
                              <span style={{ fontSize: 13, fontWeight: active ? 700 : 500, color: active ? '#166534' : N.navy, flex: 1 }}>
                                {subject}
                              </span>
                              {availableInUni && (
                                <span style={{ fontSize: 10, color: '#16a34a', fontWeight: 700, background: 'rgba(22,163,74,0.10)', padding: '2px 8px', borderRadius: 50 }}>
                                  Available
                                </span>
                              )}
                            </motion.div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' as const, marginBottom: 22 }}>
            {['✅ International Students', '🌍 Global Recognition', '🎓 Scholarships Available', '📋 Admission Open'].map(tag => (
              <span key={tag} style={{
                fontSize: 12, padding: '5px 13px', borderRadius: 50, fontWeight: 600,
                background: 'rgba(37,99,176,0.06)', border: '1px solid rgba(37,99,176,0.16)',
                color: N.navyMid,
              }}>{tag}</span>
            ))}
          </div>
...
export default function UniversityCard({ university: u, index }: Props) {
  ...
  return (
    <>
      {showModal && <UniversityModal university={u} onClose={() => setShowModal(false)} />}
        ...
        <motion.div whileHover={{ y: -2 }} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 18, flex: 1 }}>
            <div style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              background: expanded ? 'rgba(255,255,255,0.15)' : `linear-gradient(135deg, ${N.navyMid}, ${N.blueBright})`,
            ...
            <div>
              <p style={{ color: expanded ? '#ffffff' : N.navy, fontWeight: 800, fontSize: 16, margin: 0, letterSpacing: '-0.3px', lineHeight: 1.3, transition: 'color 0.3s' }}>
                {u.name}
              </p>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 5 }}>
                <span style={{ fontSize: 15 }}>{countryFlags[u.country] || '🌍'}</span>
                <span style={{ color: expanded ? 'rgba(191,219,254,0.90)' : N.blueBright, fontSize: 13, fontWeight: 700, transition: 'color 0.3s' }}>
                  {u.country}
                </span>
              </div>
            </div>
